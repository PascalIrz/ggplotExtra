...
#'
#' @param df Dataframe contenant les données IPR. Il doit contenir des variables "ipr"
#'     et "annee" ainsi qu'une variable permettant d'identifier la station ou le point de
#'     prélèvement.
#' @param var_lattice Nom de la variable servant à identifier les stations ou points.
#'     Cette variable donnera les étiquettes du graphique.
#' @param var_y Nom de la variable contenant les valeurs IPR.
...
#' @param metriques Booléen. Si FALSE (défaut) les seuils de classes sont ceux de l'IPR.
#'     Si TRUE, ils sont divisés par 7 pour s'adapter aux métriques.
...
#' @importFrom aspe gg_gerer_seuils_classes_ipr_int
#'
#' @examples
#' \dontrun{
...
#'
#' ipr_data %>%
#'   gg_temp_ipr(var_lattice = pop_libelle,
#'               var_y = ipr,
...

{
  ...
  # {
  #   df <- df %>%
  #     filter(!!var_lattice %in% station_sel)
  # }
  ...
  df <- df %>%
    mutate(
      #var_id = str_wrap(!!var_lattice, 25), # au cas où intitulés trop longs
      var_id = as.factor(!!var_lattice),
      hover = paste0(
        "<b>",
        annee,
        "</b><br>",
  ...
  df_classes <- df_classes %>%
    gg_gerer_seuils_classes_ipr_int(metriques = metriques,
                                    sup_500m = sup_500m) %>%
